         ...
         <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
                <div class="card shadow-sm border-0 h-100 text-center p-3">
                    <div class="card-body">
                        ...
                        <h5 class="card-title mb-2">{{ $data->courseAssignment->count() }}</h5>
                        <p class="text-muted small mb-3">
                          {{$data->course_name}}
                        </p>
                        <span class="badge bg-primary mb-2">
                          {{ $data->courseAssignment->count() }} {{ Str::plural('Assignments', $data->courseAssignment->count()) }}
                        </span>
                        <a href="{{route('ins_course_assignment.add',[$data->id,$data->slug])}}" class="btn btn-sm btn-outline-primary w-100 mt-2">
                            Create New Assignment
                        </a>
                        <a href="{{route('ins_course_assignment.all_topics',[$data->id,$data->slug])}}" class="btn btn-sm btn-outline-primary w-100 mt-2">
                            View All Assignment
                        </a>
                    </div>
                </div>
