...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('ins_course_assignment.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('ins_course_assignment.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
      ...
      <div class="row flex-between-center">
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
              <!-- New Button -->
            <a href="{{route('ins_course_assignment.all_data')}}">
              ...
              <!-- Filter Button -->
            <a href="{{route('ins_course_assignment.recycle')}}">
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('ins_course_assignment.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('ins_course_assignment.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('ins_course_assignment.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('ins_course_assignment.public',[$data->id, $data->slug])}}">Publish</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('ins_course_assignment.private',[$data->id, $data->slug])}}">Draft</a>
                    @endif 
                  </div>
                </div>
              </td>
