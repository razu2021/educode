...
@section('instructor_contents')
    <main>
        <div class="container">
          ...
          <form action="{{route('ins_course_batch.submit')}}" method="post" enctype="multipart/form-data">
          @csrf
            <div class="row">
                        ...
                        <div class="card-body">
                            <div class="row gx-2">
                                ...
                                {{-- end --}}
                                    ...
                                    <label class="form-label" for="position"> Batch Start Date : </label>
                                   <input type="date" name="start_date" id="start_date" class="form-control" value="{{old('start_date')}}">
                                    <label class="text-danger fw-medium">@error('position') {{$message}} @enderror</label>
                                </div>
                                {{-- end --}}
                                 <div class="col-6 mb-1">
                                    <label class="form-label" for="position"> Batch End Date : </label>
                                    <input type="date" name="end_date" id="end_date"  class="form-control" value="{{old('end_date')}}">
                                    <label class="text-danger fw-medium">@error('position') {{$message}} @enderror</label>
                                </div>
                                {{-- end --}}
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="title">Batch Description : <span class="text-danger"><i class="bi bi-star-fill"></i></span></label>
                                    <textarea class="fomr-control" name="description" id="description" cols="30" rows="10" value="{{old('description')}}">{{old('description')}}</textarea>
                                    <label class="text-danger fw-medium">@error('title') {{$message}} @enderror</label>
                                </div>
                                {{-- end --}}
