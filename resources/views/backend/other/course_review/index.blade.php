...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('course_review.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('course_review.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
      ...
      <div class="row flex-between-center">
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
              <!-- New Button -->
            <a href="{{route('course_review.add')}}">
              ...
              <!-- Filter Button -->
            <a href="{{route('course_review.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('course_review.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('course_review.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('course_review.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('course_review.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('course_review.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('course_review.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('course_review.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('course_review.public',[$data->id, $data->slug])}}">Active</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('course_review.private',[$data->id, $data->slug])}}">Deactive</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('course_review.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
