            ...
            <div class="card mb-3">
                <div class="card-body">
                  <div class="row flex-between-center">
                    ...
                    <div class="col-auto">
                        <button class="btn btn-link text-secondary p-0 me-3 fw-medium" role="button"><a href="{{route('siteemail.add')}}">Discard</a></button>
                        <button class="btn btn-primary" role="button"> <a class="text-white" href="{{route('siteemail.all')}}">All Items </a> </button>
                    </div>
                  </div>
                </div>
              </div>
          <form action="{{route('siteemail.update')}}" method="post" enctype="multipart/form-data">
          @csrf
            <div class="row">
                <div class="col-lg-8">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <div class="row gx-2">
                                ...
                                <label class="form-label" for="title">Email Type</label>
                                  <select name="email_type" id="email_type" class="form-control">
                                      <option value="{{$data->email_type}}">{{$data->email_type}}</option>
                                      <option value="primary" {{ old('email_type') == 'primary' ? 'selected' : '' }} >Primary Email</option>
                                      <option value="secondary" {{ old('email_type') == 'secondary' ? 'selected' : '' }} >Secondary Email</option>
                                      <option value="other" {{ old('other') == 'primary' ? 'selected' : '' }}>Other Email</option>
                                  </select>
                                  <label class="text-danger fw-medium">@error('email_tupe') {{$message}} @enderror</label>
                                </div>
                                <!-- end -->
                                ...
                                <div class="col-12 mb-1">
                                    ...
                                    <label class="text-danger fw-medium">@error('email') {{$message}} @enderror</label>
                                </div>
