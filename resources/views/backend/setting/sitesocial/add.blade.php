            ...
            <div class="card mb-3">
                <div class="card-body">
                  <div class="row flex-between-center">
                    ...
                    <div class="col-auto">
                        <button class="btn btn-link text-secondary p-0 me-3 fw-medium" role="button"><a href="{{route('sitesocial.add')}}">Discard</a></button>
                        <button class="btn btn-primary" role="button"> <a class="text-white" href="{{route('sitesocial.all')}}">All Items </a> </button>
                    </div>
                  </div>
                </div>
              </div>
          <form action="{{route('sitesocial.submit')}}" method="post" enctype="multipart/form-data">
          @csrf
            <div class="row">
                <div class="col-lg-8">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <div class="row gx-2">
                                ...
                                <span class="input-group-text" id="selectedIconPreview">@</span>
                                  <input type="text" name="icon" id="selectedIconInput" value="{{old('icon')}}" class="form-control" placeholder="Select icon" aria-label="Recipient’s username" aria-describedby="button-addon2">
                                  <button class="btn btn-outline-secondary" type="button" id="button-addon2" data-bs-toggle="modal" data-bs-target="#iconModal"><i class="bi bi-emoji-smile"></i></button>
                                </div>
                                <label class="text-danger fw-medium">@error('icon') {{$message}} @enderror</label>
                                
                                </div>
                                <!-- end -->
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="title">Social Title</label>
                                ...
                                <!-- end -->
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="url">Social URL</label>
                                    <input class="form-control" name="url" id="url" type="text" value="{{old('url')}}">
                                    <label class="text-danger fw-medium">@error('url') {{$message}} @enderror</label>
                                </div>
                                <!-- end -->
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="order">Social Order</label>
                                    <input class="form-control" name="order" id="order" type="number" value="{{old('order')}}">
                                    <label class="text-danger fw-medium">@error('order') {{$message}} @enderror</label>
                                </div>
                                <!-- end -->
                            </div>
                        </div>
      ...
      <div class="modal-body">
        ...
        <div class="d-flex flex-wrap" id="iconList" style="gap: 15px;">
          @foreach ($allicon as $icon)
            <div class="icon-item text-center" data-icon="{{ strtolower($icon) }}" style="cursor:pointer;" onclick="selectIcon('{{ $icon }}')">
...
<script>
  document.getElementById('iconSearch').addEventListener('keyup', function() {
    ...
    const iconItems = document.querySelectorAll('#iconList .icon-item');

    iconItems.forEach(function(item) {
      const iconClass = item.getAttribute('data-icon');
      if (iconClass.includes(searchValue)) {
