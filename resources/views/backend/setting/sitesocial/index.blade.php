...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('sitesocial.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('sitesocial.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
            ...
            <a href="{{route('sitesocial.add')}}">
              ...
              <!-- Filter Button -->
            <a href="{{route('sitesocial.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('sitesocial.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('sitesocial.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('sitesocial.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('sitesocial.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
        ...
        <table class="table table-sm fs-10 mb-0 overflow-hidden">
          <thead class="bg-200">
            <tr class="">
              ...
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">URL</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">Order</th>
          ...
          <tbody class="list" id="table-purchase-body">
            ...
            <tr class="btn-reveal-trigger">
              ...
              <td class="align-middle white-space-nowrap phone"><i class="{{$data->icon}}"></i></td>
              <td class="align-middle white-space-nowrap phone">{{$data->title}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->url}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->order}}</td>
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('sitesocial.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('sitesocial.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('sitesocial.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('sitesocial.public',[$data->id, $data->slug])}}">Active</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('sitesocial.private',[$data->id, $data->slug])}}">Deactive</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('sitesocial.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
