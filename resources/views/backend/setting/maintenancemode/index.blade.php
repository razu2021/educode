...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('maintenancemode.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('maintenancemode.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
      ...
      <div class="row flex-between-center">
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
              <!-- New Button -->
            <a href="{{route('maintenancemode.add')}}">
              ...
              <!-- Filter Button -->
            <a href="{{route('maintenancemode.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('maintenancemode.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('maintenancemode.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('maintenancemode.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('maintenancemode.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
        ...
        <table class="table table-sm fs-10 mb-0 overflow-hidden">
          <thead class="bg-200">
            <tr class="">
              ...
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">caption</th>
          ...
          <tbody class="list" id="table-purchase-body">
            ...
            <tr class="btn-reveal-trigger">
              ...
              <td class="align-middle white-space-nowrap email">{{$data->title}}</td>
              <td class="align-middle white-space-nowrap product">{!! Str::words($data->caption,10) !!}</td>
              ...
              <td class="align-middle text-center fs-9 white-space-nowrap payment">
                @if ($data->public_status === 1)
                  <small class="badge rounded badge-subtle-success bg-success text-white">Published</small>
                @elseif($data->public_status === 2)
                <small class="badge rounded badge-subtle-success bg-success  text-white">Published</small>
                @else
                  <small class="badge rounded badge-subtle-danger bg-danger text-white false">Disable </small>
                @endif
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('maintenancemode.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('maintenancemode.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('maintenancemode.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('maintenancemode.public',[$data->id, $data->slug])}}">Enable</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('maintenancemode.private',[$data->id, $data->slug])}}">Disable</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('maintenancemode.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
                    <a class="dropdown-item" href="javascript::voied(0)" data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$data->id}}"> Appy Change</a>
    ...
    <div class="modal-content border-0">
      ...
      <div class="modal-body p-0">
        <div class="rounded-top-3 bg-body-tertiary py-3 ps-4 pe-6">
          <h4 class="mb-1" id="staticBackdropLabel">Activate Maintenance Mode as your Needs!</h4>
          ...
          <div class="row">
            <div class="col-lg-8">
              ...
              <div class="d-flex"><span class="fa-stack ms-n1 me-3"><i class="fas fa-circle fa-stack-2x text-200"></i><i class="fa-inverse fa-stack-1x text-primary fas fa-align-left" data-fa-transform="shrink-2"></i></span>
                <div class="flex-1">
                  <h5 class="mb-2 fs-9">Maintenance Title </h5>
                  <p class="text-break fs-10"> {{$Data->title ?? "No Data Found"}}</p>
                </div>
              </div>
              <hr class="my-4" />
              <div class="d-flex"><span class="fa-stack ms-n1 me-3"><i class="fas fa-circle fa-stack-2x text-200"></i><i class="fa-inverse fa-stack-1x text-primary fas fa-align-left" data-fa-transform="shrink-2"></i></span>
                <div class="flex-1">
                  ...
                  <p class="text-break fs-10">{!! $Data->caption !!} </p>
                </div>
              </div>
            </div>
            <div class="col-lg-4">
              <h6 class="mt-5 mt-lg-0">Select Mode </h6>
              <ul class="nav flex-lg-column fs-10">
                @if($Data->public_status === 0)
                <li class="nav-item me-2 me-lg-0"><a class="nav-link nav-link-card-details bg-success text-white" href="{{route('maintenancemode.public',[$Data->id, $Data->slug])}}">Enable Entair Website </a></li>
                <li class="nav-item me-2 me-lg-0"><a class="nav-link nav-link-card-details bg-success text-white" href="{{route('maintenancemode.public_header',[$Data->id, $Data->slug])}}">Apply in Header </a></li>
                @elseif($Data->public_status === 1)
                <li class="nav-item me-2 me-lg-0"><a class="nav-link nav-link-card-details bg-danger text-white" href="{{route('maintenancemode.private',[$Data->id, $Data->slug])}}">Maintenance Mode Disable </a></li>
                @elseif($Data->public_status === 2)
                <li class="nav-item me-2 me-lg-0"><a class="nav-link nav-link-card-details bg-danger text-white" href="{{route('maintenancemode.private_header',[$Data->id, $Data->slug])}}">Maintenance Mode Disable </a></li>
                @endif
