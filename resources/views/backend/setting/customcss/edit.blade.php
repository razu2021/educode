            ...
            <div class="card mb-3">
                <div class="card-body">
                  <div class="row flex-between-center">
                    ...
                    <div class="col-auto">
                        <button class="btn btn-link text-secondary p-0 me-3 fw-medium" role="button"><a href="{{route('customcss.add')}}">Discard</a></button>
                        <button class="btn btn-primary" role="button"> <a class="text-white" href="{{route('customcss.all')}}">All Items </a> </button>
                    </div>
                  </div>
                </div>
              </div>
          <form action="{{route('customcss.update')}}" method="post" enctype="multipart/form-data">
          @csrf
            <div class="row">
                <div class="col-lg-8">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <div class="row gx-2">
                                ...
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="script_note">Css Note:</label>
                                   <input type="text" name="css_note" id="css_note" class="form-control" value="{{$data->css_note}}">
                                    <label class="text-danger fw-medium">@error('css_note') {{$message}} @enderror</label>
                                </div>
                              <!-- end -->

                                <div class="col-12 mb-1">
                                    <label class="form-label" for="address">Address:</label>
                                    <textarea name="custom_css" id="custom_css" class="form-control" rows="10"  value="{{$data->custom_css}}">{{$data->custom_css}}</textarea>
                                    <label class="text-danger fw-medium">@error('custom_css') {{$message}} @enderror</label>
                                </div>
