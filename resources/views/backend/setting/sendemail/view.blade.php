            ...
            <div class="card mb-3">
                <div class="card-body">
                  <div class="row flex-between-center">
                    ...
                    <div class="col-auto">
                        <button class="btn btn-link text-secondary p-0 me-3 fw-medium" role="button"><a href="{{route('sendemail.add')}}">Discard</a></button>
                        <button class="btn btn-primary" role="button"> <a class="text-white" href="{{route('sendemail.all')}}">All Items </a> </button>
                    </div>
                  </div>
                </div>
              </div>
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                      @if (!empty($data->mail_attachment))
                                        @php
                                          $mail_att = json_decode($data->mail_attachment, true); // true দিলে অ্যারে আকারে পাবে
                                        @endphp

                                          {{-- attachment list end  --}}
                                          <ol>
                                             @foreach ($mail_att as $mailattach)
                                            <li>
                                              <a href="{{ asset('storage/uploads/email/attachments/' . $mailattach) }}" target="_blank" download>
                                               Download :  {{ $mailattach }}
                                              </a>
                                            </li>
                                             @endforeach
