...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('sendemail.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('sendemail.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
      ...
      <div class="row flex-between-center">
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
              <!-- New Button -->
            <a href="{{route('sendemail.add')}}">
              <button class="btn btn-falcon-default btn-sm" type="button">
                <i class="fas fa-plus"></i>
                <span class="d-none d-sm-inline-block ms-1">Compose</span>
              </button>
            </a>

              <!-- Filter Button -->
            <a href="{{route('sendemail.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('sendemail.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('sendemail.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('sendemail.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('sendemail.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
        ...
        <table class="table table-sm fs-10 mb-0 overflow-hidden">
          <thead class="bg-200">
            <tr class="">
              ...
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">To</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">Subject</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">Body</th>
          ...
          <tbody class="list" id="table-purchase-body">
            ...
            <tr class="btn-reveal-trigger">
              ...
              <td class="align-middle white-space-nowrap phone">{{$data->mail_to}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->mail_subject}}</td>
              <td class="align-middle white-space-nowrap phone">{!! Str::words($data->mail_subject,10) !!}</td>
              ...
              <td class="align-middle text-center fs-9 white-space-nowrap payment">
                @if ($data->public_status === 1)
                  <small class="badge rounded badge-subtle-success bg-success text-white">Send</small>
                @else
                  <small class="badge rounded badge-subtle-danger bg-danger text-white false">Faild </small>
                @endif
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('sendemail.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('sendemail.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('sendemail.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('sendemail.public',[$data->id, $data->slug])}}">Active</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('sendemail.private',[$data->id, $data->slug])}}">Deactive</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('sendemail.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
