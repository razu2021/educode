...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('pgcredential.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('pgcredential.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
            ...
            <a href="{{route('pgcredential.add')}}">
              ...
              <!-- Filter Button -->
            <a href="{{route('pgcredential.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('pgcredential.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('pgcredential.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('pgcredential.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('pgcredential.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
        ...
        <table class="table table-sm fs-10 mb-0 overflow-hidden">
          <thead class="bg-200">
            <tr class="">
              ...
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">Gateway Type</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="des">Gateway Name</th>
          ...
          <tbody class="list" id="table-purchase-body">
            ...
            <tr class="btn-reveal-trigger">
              ...
              <td class="align-middle white-space-nowrap phone">{{$data->gateway_type}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->gateway_name}}</td>
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('pgcredential.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('pgcredential.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('pgcredential.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('pgcredential.public',[$data->id, $data->slug])}}">Active</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('pgcredential.private',[$data->id, $data->slug])}}">Deactive</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('pgcredential.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
