...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('embedmap.bulkAction') }}";
  const csrfToken = "{{ csrf_token() }}";
</script>
@endpush

<div class="card z-1 mb-3" id="recentPurchaseTable" data-list="{&quot;valueNames&quot;:[&quot;name&quot;,&quot;phone&quot;,&quot;product&quot;,&quot;payment&quot;,&quot;amount&quot;],&quot;page&quot;:8,&quot;pagination&quot;:true}">
    <div class="card-header">
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('embedmap.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
      ...
      <div class="row flex-between-center">
        <div class="col-6 col-sm-auto d-flex align-items-center pe-0">
          <h5 class="fs-9 mb-0 text-nowrap py-2 py-xl-0">All Infomations </h5>
        </div>
            ...
            <a href="{{route('embedmap.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('embedmap.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('embedmap.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('embedmap.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('embedmap.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
        ...
        <table class="table table-sm fs-10 mb-0 overflow-hidden">
          <thead class="bg-200">
            <tr class="">
              ...
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="title">Address Line1</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="title">Address Line2</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="title">City</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="title">State</th>
              <th class="text-900 sort pe-1 align-middle white-space-nowrap" data-sort="title">Postal Code</th>
          ...
          <tbody class="list" id="table-purchase-body">
            ...
            <tr class="btn-reveal-trigger">
              ...
              <td class="align-middle white-space-nowrap phone">{{$data->address_line1}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->address_line2}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->city}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->state}}</td>
              <td class="align-middle white-space-nowrap phone">{{$data->postal_code}}</td> 
              ...
              <td class="align-middle text-center fs-9 white-space-nowrap payment">
                ...
                @else
                  <small class="badge rounded badge-subtle-danger bg-danger text-white false">Deactive </small>
                @endif
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('embedmap.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('embedmap.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('embedmap.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('embedmap.public',[$data->id, $data->slug])}}">Active</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('embedmap.private',[$data->id, $data->slug])}}">Deactive</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('embedmap.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
