...
@push('scripts')
<script>
  const bulkActionUrl = "{{ route('coursecategory.bulkAction') }}";
            ...
            <div class="col-md-12 ">
                <form action="" method="get">
                  <div class="input-group">
                    ...
                    <a href="{{route('coursecategory.all')}}"><button class="btn btn-outline-primary" type="button">Reset</button></a>
                  </div>
                </form>
            </div>
            {{-- col end --}}
      ...
      <div class="row flex-between-center">
        ...
        <div class="col-6 col-sm-auto ms-auto text-end ps-0">
          ...
          <div id="table-purchases-replace-element" class="d-flex align-items-center">
              <!-- New Button -->
            <a href="{{route('coursecategory.add')}}">
              ...
              <!-- Filter Button -->
            <a href="{{route('coursecategory.recycle')}}">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                  <li><a class="dropdown-item" href="{{route('coursecategory.export_pdf')}}">Export as PDF</a></li>
                  <li><a class="dropdown-item" href="{{route('coursecategory.export_excel')}}">Export as Excel</a></li>
                  <li><a class="dropdown-item" href="{{route('coursecategory.export_csv')}}">Export as CSV</a></li>
                  <li><a class="dropdown-item" href="{{route('coursecategory.export_zip')}}">Export as Zip</a></li>
                </ul>
              </div>
        ...
        <table class="table table-sm fs-10 mb-0 overflow-hidden">
          ...
          <tbody class="list" id="table-purchase-body">
            ...
            <tr class="btn-reveal-trigger">
              ...
              <td class="align-middle white-space-nowrap email">{{$data->course_category_name}}</td>
              <td class="align-middle white-space-nowrap product">{{$data->course_category_title}}</td>
              <td class="align-middle white-space-nowrap product">{{Str::words($data->course_category_des,10)}}</td>
              ...
              <td class="align-middle white-space-nowrap text-end">
                <div class="dropstart font-sans-serif position-static d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-recent-purchase-table-0">
                    <a class="dropdown-item" href="{{route('coursecategory.view',[$data->id, $data->slug])}}">View</a>
                    <a class="dropdown-item" href="{{route('coursecategory.edit',[$data->id, $data->slug])}}">Edit</a>
                    <!-- Hidden form to submit DELETE request -->
                    <form id="deleteForm{{ $data->id }}" action="{{ route('coursecategory.softdelete', $data->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
                    ...
                    @if($data->public_status === 0)
                      <a class="dropdown-item text-success" href="{{route('coursecategory.public',[$data->id, $data->slug])}}">Publish</a>
                    @else 
                     <a class="dropdown-item text-warning" href="{{route('coursecategory.private',[$data->id, $data->slug])}}">Private</a>
                    @endif 
                    <a class="dropdown-item" href="{{route('coursecategory.export_single_pdf',[$data->id, $data->slug])}}">Export PDF</a>
