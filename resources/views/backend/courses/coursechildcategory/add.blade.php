            ...
            <div class="card mb-3">
                <div class="card-body">
                  <div class="row flex-between-center">
                    <div class="col-md">
                      <h5 class="mb-2 mb-md-0">Add a New Infomation</h5>
                    </div>
                    <div class="col-auto">
                        <button class="btn btn-link text-secondary p-0 me-3 fw-medium" role="button"><a href="{{route('coursechildcategory.add')}}">Discard</a></button>
                        <button class="btn btn-primary" role="button"> <a class="text-white" href="{{route('coursechildcategory.all')}}">All Items </a> </button>
                    </div>
                  </div>
                </div>
              </div>
          <form action="{{route('coursechildcategory.submit')}}" method="post" enctype="multipart/form-data">
          @csrf
            <div class="row">
                <div class="col-lg-8">
                    <div class="card mb-3">
                            ...
                            <div class="row gx-2">
                              <div class="col-12 mb-1">
                                <label for="category">Select Course Category</label>
                                  <select id="categoryDropdown" name="course_category_id" class="form-control" disabled>
                                    <option value="">Select Course Category</option>
                                    @foreach($courseCategory as $categorys)
                                        <option value="{{ $categorys->id }}">{{ $categorys->course_category_name }}</option>
                                    @endforeach
                                  </select>
                              </div>
                              <input type="hidden" name="course_category_id" id="hiddenCategoryInput">
                              <!-- Hidden input to actually POST the selected category_id -->
                             
                              <div class="col-12 mb-1">
                                <label for="category">Select Category</label>
                                  <select id="subcategoryDropdown" name="course_subcategory_id" class="form-control">
                                    <option value="">Select Subcategory</option>
                                    @foreach($coursesubCategory as $subcategorys)
                                        <option value="{{ $subcategorys->id }}" data-category="{{ $subcategorys->course_category_id}}">
                                            {{ $subcategorys->course_sub_category_name }}
                                        </option>
                                    @endforeach
                                  </select>
                                ...
                                {{-- end subcategory --}}
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="course_sub_category_name">Child Categorie Name:</label>
                                    <input class="form-control" name="course_child_category_name" id="course_sub_category_name" type="text" value="{{old('course_sub_category_name')}}">
                                    <label class="text-danger fw-medium">@error('course_child_category_name') {{$message}} @enderror</label>
                                </div>
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="course_sub_category_title">Chid Categorie Title:</label>
                                    <input class="form-control" name="course_child_category_title" id="course_sub_category_title" type="text" value="{{old('course_sub_category_title')}}">
                                    <label class="text-danger fw-medium">@error('course_child_category_title') {{$message}} @enderror</label>
                                </div>
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="course_sub_category_desc">Child Categorie Descriptions:</label>
                                    <input class="form-control" name="course_child_category_desc" id="course_sub_category_desc" type="text" value="{{old('course_sub_category_desc')}}">
                                    <label class="text-danger fw-medium" >@error('course_child_category_desc') {{$message}} @enderror</label>
                                </div>
                            </div>
                        </div>
                      </div>
                      ...
                      <div class="card">
                          <div class="card-body">
                            <div class="col-12">
                              ...
                              <label class="text-danger fw-medium" >@error('custom_url') {{$message}} @enderror</label>
