                            ...
                            <div class="row gx-2">
                              ...
                              <!-- end -->
                                <div class="col-12 mb-1">
                                    ...
                                    <input class="form-control" name="short_des" id="short_des" type="text" value="{{$data->short_des}}">
                                    <label class="text-danger fw-medium">@error('short_des') {{$message}} @enderror</label>
                                </div>
                              <!-- end -->
                                <div class="col-12 mb-1">
                                    <label class="form-label" for="description"> Description</label>
                                  <textarea name="description" id="description" cols="30" rows="10" value="{{$data->description}}">{!! $data->description!!}</textarea>
                                    <label class="text-danger fw-medium">@error('description') {{$message}} @enderror</label>
                                </div>
                              <!-- end -->
                               <div class="col-6 mb-1">
                                    <label class="form-label" for="short_des">Short Description</label>
                                    <input class="form-control" name="images" id="imaes" type="file" accept="image/*"  onchange="document.getElementById('previewImage').src = window.URL.createObjectURL(this.files[0])">
                                    <label class="text-danger fw-medium">@error('short_des') {{$message}} @enderror</label>
                              </div>
                               <div class="col-6 mb-1 text-end">
                                  @if ($data->image !== '')
                                     <img id="previewImage" src="{{asset($data->image)}}" alt="post image" width="200" height="200">
                                  @else
                                    <img id="previewImage" src="{{asset('contents/backend/avater.jpg')}}" alt="banner_avater" width="200" height="200">
                                  @endif
                              </div>
