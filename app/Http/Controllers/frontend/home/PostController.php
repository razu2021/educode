...
use App\Exports\ExportData\PostExport;
use ZipArchive;
...
class PostController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = post::where('status',1)->where('title','LIKE',"%$search%")
           ->get();
        }else{
            $all = post::where('status',1)->get();
        }
        return view('backend.frontend.post..index',compact('all'));
    }
    ...
    **/
    public function add(){
        ...
        return view('backend.frontend.post..add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= post::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.frontend.post..view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = post::latest()->first();
        $data= post::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.frontend.post..edit',compact('totalpost','latestPost','data'));
    }
 ...
 /**  ------- upload image using image intervention -------- use image top */
        if($request->hasFile('images')){
            ...
            $realPath = 'uploads/post/';  // make real path for store name in database
            ...
            post::where('id',$insert->id)->where('slug',$slug)->update([
                'image'=>  $fullPath,
            ]);
        }
   ...
   /**
    ...
    if($request->hasFile('images')){
        ...
        $old_data = post::where('id', $id)->first();
        $file_paths = public_path($old_data->image);
        ...
        /**-- save image name in  database */
        post::where('id',$id)->where('slug',$slug)->update([
            'image'=>  $fullPath,
        ]);
    }
    // ----  image upload end here 
    ...
    **/
    public function public_status($id,$slug){
        $published = post::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = post::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        ...
        return view('backend.frontend.post..recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'heard_delete') {
            $categories = post::onlyTrashed()->whereIn('id', $ids)->get();
        ...
        if ($action === 'active') {
            $categories = post::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    post::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    post::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = post::get();
       // return view('backend.frontend.post..export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.frontend.post..export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.frontend.post..export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.frontend.post..export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new PostExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new PostExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new PostExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new PostExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.frontend.post..export_pdf', ['categories' => post::all()]);
