...
use App\Exports\ExportData\CopyrightContentExport;
use ZipArchive;
...
class CopyRightController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = CopyRight::where('status',1)->where('address','LIKE',"%$search%")
           ->get();
        }else{
            $all = CopyRight::where('status',1)->get();
        }
        return view('backend.setting.copyright.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = CopyRight::get()->count();
        $latestPost = CopyRight::latest()->first();
        return view('backend.setting.copyright.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= CopyRight::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.copyright.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = CopyRight::latest()->first();
        $data= CopyRight::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.copyright.edit',compact('totalpost','latestPost','data'));
    }
    ...
    **/
    public function restore($id){
        $data = CopyRight::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function public_status($id,$slug){
        $published = CopyRight::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = CopyRight::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = CopyRight::onlyTrashed()->get();
        return view('backend.setting.copyright.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = CopyRight::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    CopyRight::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    CopyRight::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = CopyRight::get();
       // return view('backend.setting.copyright.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.copyright.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.setting.copyright.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.copyright.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new CopyrightContentExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new CopyrightContentExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new CopyrightContentExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new CopyrightContentExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.copyright.export_pdf', ['categories' => CopyRight::all()]);
