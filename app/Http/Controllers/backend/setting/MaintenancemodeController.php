...
use App\Exports\ExportData\MaintenanceModeExport;
use ZipArchive;
...
class MaintenancemodeController extends Controller
{
 ...
 /**
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = MaintenanceMode::where('status',1)->where('title','LIKE',"%$search%")
            ->orWhere('caption','LIKE',"%$search%")->get();
        }else{
            $all = MaintenanceMode::where('status',1)->get();
        }
        return view('backend.setting.maintenancemode.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = MaintenanceMode::get()->count();
        $latestPost = MaintenanceMode::latest()->first();
        return view('backend.setting.maintenancemode.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= MaintenanceMode::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.maintenancemode.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = MaintenanceMode::latest()->first();
        $data= MaintenanceMode::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.maintenancemode.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    if($update){
        ...
        return redirect()->route('maintenancemode.view',[$id,$slug]);
    }else{
    ...
    **/
    public function restore($id){
        $data = MaintenanceMode::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function public_status($id,$slug){
        $published = MaintenanceMode::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = MaintenanceMode::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    * ---------  Private post  functionality --------//
    **/
    public function public_header($id,$slug){
        $private = MaintenanceMode::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>2,
        ]);
        // Delete Successfully 
        if($private){
            flash()->success('Information Update Successfully !');
        }else{
            flash()->error('Informatin Private Faild !');
        }
        return redirect()->back();
    ...
    * ---------  Private post  functionality --------//
    **/
    public function private_header($id,$slug){
        $private = MaintenanceMode::where('id',$id)->where('slug',$slug)->where('public_status',2)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = MaintenanceMode::onlyTrashed()->get();
        return view('backend.setting.maintenancemode.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = MaintenanceMode::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    MaintenanceMode::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    MaintenanceMode::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = MaintenanceMode::get();
       // return view('backend.setting.maintenancemode.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.maintenancemode.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.setting.maintenancemode.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.maintenancemode.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new MaintenanceModeExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new MaintenanceModeExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new MaintenanceModeExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new MaintenanceModeExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.maintenancemode.export_pdf', ['categories' => MaintenanceMode::all()]);
