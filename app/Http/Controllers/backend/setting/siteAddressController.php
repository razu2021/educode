...
use App\Exports\ExportData\SiteAddressExport;
use ZipArchive;
...
class siteAddressController extends Controller
{
 /**
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = SiteAddress::where('status',1)->where('address','LIKE',"%$search%")
           ->get();
        }else{
            $all = SiteAddress::where('status',1)->get();
        }
        return view('backend.setting.siteaddress.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = SiteAddress::get()->count();
        $latestPost = SiteAddress::latest()->first();
        return view('backend.setting.siteaddress.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= SiteAddress::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.siteaddress.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = SiteAddress::latest()->first();
        $data= SiteAddress::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.siteaddress.edit',compact('totalpost','latestPost','data'));
    }
    ...
    **/
    public function restore($id){
        $data = SiteAddress::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function public_status($id,$slug){
        $published = SiteAddress::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = SiteAddress::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = SiteAddress::onlyTrashed()->get();
        return view('backend.setting.siteaddress.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'heard_delete') {
            $categories = SiteAddress::onlyTrashed()->whereIn('id', $ids)->get();
        ...
        if ($action === 'active') {
            $categories = SiteAddress::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    SiteAddress::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    SiteAddress::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = SiteAddress::get();
       // return view('backend.setting.siteaddress.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.siteaddress.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.setting.siteaddress.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.siteaddress.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new SiteAddressExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new SiteAddressExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new SiteAddressExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new SiteAddressExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.siteaddress.export_pdf', ['categories' => SiteAddress::all()]);
