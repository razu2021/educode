...
use App\Exports\ExportData\SiteAdminmailExport;
use ZipArchive;
...
class SiteAdminMailController extends Controller
{
...
/**
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = SiteAdminMail::where('status',1)->where('mail','LIKE',"%$search%")
           ->get();
        }else{
            $all = SiteAdminMail::where('status',1)->get();
        }
        return view('backend.setting.siteadminmail.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = SiteAdminMail::get()->count();
        $latestPost = SiteAdminMail::latest()->first();
        return view('backend.setting.siteadminmail.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= SiteAdminMail::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.siteadminmail.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = SiteAdminMail::latest()->first();
        $data= SiteAdminMail::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.siteadminmail.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        /**--- validation code -- */
        $request->validate([
            'mail' => ['required','email','unique:'.SiteAdminMail::class],
        ], [
            'mail.required' => 'Email is Required!',
            'mail.email' => 'Please enter a valid email address.',
            'mail.unique' => 'This email address is already registered.',
        ]);
   ...
   /**
    ...
    //---------category update -------//
    $update = SiteAdminMail::where('id',$id)->where('slug',$slug)->update([
        'mail_user'=>$request->mail_user,
        'mail'=>$request->mail,
        'editor_id' => $editor,
    ...
    if($update){
        ...
        return redirect()->route('siteadminmail.view',[$id,$slug]);
    }else{
   ...
   /**
    ...
    public function softdelete($id){
        $data= SiteAdminMail::where('id',$id)->first();
        $data->delete();
        // ----Delete Successfully ----//
        if($data){
    ...
    **/
    public function restore($id){
        $data = SiteAdminMail::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = SiteAdminMail::onlyTrashed()->where('id', $id)->first();
        if($data) {
            $data->forceDelete();
    ...
    **/
    public function public_status($id,$slug){
        $published = SiteAdminMail::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = SiteAdminMail::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = SiteAdminMail::onlyTrashed()->get();
        return view('backend.setting.siteadminmail.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = SiteAdminMail::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    SiteAdminMail::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    SiteAdminMail::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = SiteAdminMail::get();
       // return view('backend.setting.siteadminmail.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.siteadminmail.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = SiteAdminMail::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.setting.siteadminmail.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.siteadminmail.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new SiteAdminmailExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new SiteAdminmailExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new SiteAdminmailExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new SiteAdminmailExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.siteadminmail.export_pdf', ['categories' => SiteAdminMail::all()]);
