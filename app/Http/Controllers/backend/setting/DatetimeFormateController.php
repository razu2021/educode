...
use App\Exports\ExportData\DatetimeFormateExport;
use ZipArchive;
...
class DatetimeFormateController extends Controller
{
...
/**
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = DateTimeFormate::where('status',1)->where('date_formate','LIKE',"%$search%")
           ->get();
        }else{
            $all = DateTimeFormate::where('status',1)->get();
        }
        return view('backend.setting.datetime.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = DateTimeFormate::get()->count();
        $latestPost = DateTimeFormate::latest()->first();
        return view('backend.setting.datetime.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= DateTimeFormate::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.datetime.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = DateTimeFormate::latest()->first();
        $data= DateTimeFormate::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.datetime.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        ...
        $request->validate([
                ...
                'date_formate.required'=> ' Date Formate  is Required !',
                'time_formate.required'=> ' Date  Formate is Required !',
            ]
        );
   ...
   /**
    ...
    //---------category update -------//
    $update = DateTimeFormate::where('id',$id)->where('slug',$slug)->update([
        'date_formate'=>$request->date_formate,
        'time_formate'=>$request->time_formate,
        'editor_id' => $editor,
   ...
   /**
    ...
    public function softdelete($id){
        $data= DateTimeFormate::where('id',$id)->first();
        $data->delete();
        // ----Delete Successfully ----//
        if($data){
    ...
    **/
    public function restore($id){
        $data = DateTimeFormate::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = DateTimeFormate::onlyTrashed()->where('id', $id)->first();
        if($data) {
            $data->forceDelete();
    ...
    **/
    public function public_status($id,$slug){
        $published = DateTimeFormate::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = DateTimeFormate::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = DateTimeFormate::onlyTrashed()->get();
        return view('backend.setting.datetime.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = DateTimeFormate::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    DateTimeFormate::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    DateTimeFormate::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = DateTimeFormate::get();
       // return view('backend.setting.datetime.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.datetime.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = DateTimeFormate::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.setting.datetime.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.datetime.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new DatetimeFormateExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new DatetimeFormateExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new DatetimeFormateExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new DatetimeFormateExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.datetime.export_pdf', ['categories' => DateTimeFormate::all()]);
