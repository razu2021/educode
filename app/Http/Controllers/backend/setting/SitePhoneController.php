...
use App\Exports\ExportData\SitePhoneExport;
use ZipArchive;
...
class SitePhoneController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = SitePhone::where('status',1)->where('phone_type','LIKE',"%$search%")
            ->orWhere('phone','LIKE',"%$search%")->get();
        }else{
            $all = SitePhone::where('status',1)->get();
        }
        return view('backend.setting.sitephone.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = SitePhone::get()->count();
        $latestPost = SitePhone::latest()->first();
        return view('backend.setting.sitephone.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= SitePhone::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.sitephone.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = SitePhone::latest()->first();
        $data= SitePhone::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.sitephone.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        ...
        $request->validate([
                ...
                'phone_type.required'=> 'phone Type  is Required !',
   ...
   /**
    ...
    if($update){
        ...
        return redirect()->route('sitephone.view',[$id,$slug]);
    }else{
    ...
    **/
    public function restore($id){
        $data = SitePhone::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function public_status($id,$slug){
        $published = SitePhone::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = SitePhone::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = SitePhone::onlyTrashed()->get();
        return view('backend.setting.sitephone.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = SitePhone::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    SitePhone::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    SitePhone::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = SitePhone::get();
       // return view('backend.setting.sitephone.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.sitephone.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.setting.sitephone.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.sitephone.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new SitePhoneExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new SitePhoneExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new SitePhoneExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new SitePhoneExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.sitephone.export_pdf', ['categories' => SitePhone::all()]);
