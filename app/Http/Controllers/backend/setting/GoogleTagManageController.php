...
use App\Exports\ExportData\GoogletagMangerExport;
use ZipArchive;
...
class GoogleTagManageController extends Controller
{
     /**
     * =============================================================
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = GoogleTagManager::where('status',1)->where('tag_manager_id','LIKE',"%$search%")
           ->get();
        }else{
            $all = GoogleTagManager::where('status',1)->get();
        }
        return view('backend.setting.googletagmanager.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = GoogleTagManager::get()->count();
        $latestPost = GoogleTagManager::latest()->first();
        return view('backend.setting.googletagmanager.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= GoogleTagManager::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.googletagmanager.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = GoogleTagManager::latest()->first();
        $data= GoogleTagManager::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.googletagmanager.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        /**--- validation code -- */
        $request->validate([
                'tag_manager_id'=> 'required',
            ],[
                'tag_manager_id.required'=> ' phone  is Required !',
            ]
        );
        ...
        $insert = GoogleTagManager::create([
   ...
   /**
    ...
    //---------category update -------//
    $update = GoogleTagManager::where('id',$id)->where('slug',$slug)->update([
        'tag_manager_id'=>$request->tag_manager_id,
        'editor_id' => $editor,
    ...
    if($update){
        ...
        return redirect()->route('googletagmanager.view',[$id,$slug]);
    }else{
   ...
   /**
    ...
    public function softdelete($id){
        $data= GoogleTagManager::where('id',$id)->first();
        $data->delete();
        // ----Delete Successfully ----//
        if($data){
    ...
    **/
    public function restore($id){
        $data = GoogleTagManager::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = GoogleTagManager::onlyTrashed()->where('id', $id)->first();
        if($data) {
            $data->forceDelete();
    ...
    **/
    public function public_status($id,$slug){
        $published = GoogleTagManager::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = GoogleTagManager::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = GoogleTagManager::onlyTrashed()->get();
        return view('backend.setting.googletagmanager.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = GoogleTagManager::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    GoogleTagManager::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    GoogleTagManager::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = GoogleTagManager::get();
       // return view('backend.setting.googletagmanager.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.googletagmanager.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = GoogleTagManager::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.setting.googletagmanager.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.googletagmanager.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new GoogletagMangerExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new GoogletagMangerExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new GoogletagMangerExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new GoogletagMangerExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.googletagmanager.export_pdf', ['categories' => GoogleTagManager::all()]);
