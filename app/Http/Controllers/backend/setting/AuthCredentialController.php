...
use App\Exports\ExportData\AuthcredentialsExport;
use ZipArchive;
...
/**
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = AuthCredentials::where('status',1)->where('name','LIKE',"%$search%")
           ->get();
        }else{
            $all = AuthCredentials::where('status',1)->get();
        }
        return view('backend.setting.authcredential.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = AuthCredentials::get()->count();
        $latestPost = AuthCredentials::latest()->first();
        return view('backend.setting.authcredential.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= AuthCredentials::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.authcredential.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = AuthCredentials::latest()->first();
        $data= AuthCredentials::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.authcredential.edit',compact('totalpost','latestPost','data'));
    }
   /**
    * ---------  manage all  page functionality --------
    **/
    public function manage_all(){
        $alldata= AuthCredentials::where('status',1)->get();
        return view('backend.setting.authcredential.manageall',compact('alldata'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        ...
        $request->validate([
                ...
                'name.required'=> 'Name is Requred !',
                'client_id.required'=> 'Client Id is Requred !',
                'client_secret.required'=> 'Client Secret or Api key  is Requred !',
                'redirect_url.required'=> 'Redirect url is Requred !',
            ]
        );
   ...
   /**
    ...
    //---------category update -------//
    $update = AuthCredentials::where('id',$id)->where('slug',$slug)->update([
        'name'=>$request->name,
        'client_id'=>$request->client_id,
        'client_secret'=>$request->client_secret,
        'redirect_url'=>$request->redirect_url,
    ...
    if($update){
        ...
        return redirect()->route('authcredential.view',[$id,$slug]);
    }else{
   ...
   /**
    ...
    public function softdelete($id){
        $data= AuthCredentials::where('id',$id)->first();
        $data->delete();
        // ----Delete Successfully ----//
        if($data){
    ...
    **/
    public function restore($id){
        $data = AuthCredentials::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = AuthCredentials::onlyTrashed()->where('id', $id)->first();
        if($data) {
            $data->forceDelete();
    ...
    **/
    public function public_status($id,$slug){
        $published = AuthCredentials::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = AuthCredentials::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = AuthCredentials::onlyTrashed()->get();
        return view('backend.setting.authcredential.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = AuthCredentials::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    AuthCredentials::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    AuthCredentials::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = AuthCredentials::get();
       // return view('backend.setting.authcredential.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.authcredential.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = AuthCredentials::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.setting.authcredential.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.authcredential.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new AuthcredentialsExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new AuthcredentialsExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new AuthcredentialsExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new AuthcredentialsExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.authcredential.export_pdf', ['categories' => AuthCredentials::all()]);
