...
use App\Exports\ExportData\CustomcssExport;
use ZipArchive;
...
class CustomCssController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = CustomCss::where('status',1)->where('custom_css','LIKE',"%$search%")
           ->get();
        }else{
            $all = CustomCss::where('status',1)->get();
        }
        return view('backend.setting.customcss.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = CustomCss::get()->count();
        $latestPost = CustomCss::latest()->first();
        return view('backend.setting.customcss.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= CustomCss::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.customcss.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = CustomCss::latest()->first();
        $data= CustomCss::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.customcss.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        ...
        $request->validate([
                ...
                'custom_css.required'=> ' Custom css  is Required !',
            ]
        );
   ...
   /**
    ...
    public function update(Request $request){
        ...
        $request->validate([
            ...
            'custom_css.required'=> ' Custome Css is Required !',
        ]
    );
    ...
    if($update){
        ...
        return redirect()->route('customcss.view',[$id,$slug]);
    }else{
    ...
    **/
    public function restore($id){
        $data = CustomCss::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function public_status($id,$slug){
        $published = CustomCss::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = CustomCss::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = CustomCss::onlyTrashed()->get();
        return view('backend.setting.customcss.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = CustomCss::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    CustomCss::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    CustomCss::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = CustomCss::get();
       // return view('backend.setting.customcss.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.customcss.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.setting.customcss.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.customcss.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new CustomcssExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new CustomcssExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new CustomcssExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new CustomcssExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.customcss.export_pdf', ['categories' => CustomCss::all()]);
