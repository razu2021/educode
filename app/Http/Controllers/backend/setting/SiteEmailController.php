...
use App\Exports\ExportData\SiteEmailExport;
use ZipArchive;
...
class SiteEmailController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = SiteEmail::where('status',1)->where('email_type','LIKE',"%$search%")
            ->orWhere('email','LIKE',"%$search%")->get();
        }else{
            $all = SiteEmail::where('status',1)->get();
        }
        return view('backend.setting.siteemail.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = SiteEmail::get()->count();
        $latestPost = SiteEmail::latest()->first();
        return view('backend.setting.siteemail.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= SiteEmail::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.siteemail.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = SiteEmail::latest()->first();
        $data= SiteEmail::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.setting.siteemail.edit',compact('totalpost','latestPost','data'));
    }
    ...
    **/
    public function restore($id){
        $data = SiteEmail::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function public_status($id,$slug){
        $published = SiteEmail::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = SiteEmail::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = SiteEmail::onlyTrashed()->get();
        return view('backend.setting.siteemail.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = SiteEmail::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    SiteEmail::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    SiteEmail::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = SiteEmail::get();
       // return view('backend.setting.siteemail.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.siteemail.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.setting.siteemail.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.setting.siteemail.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new SiteEmailExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new SiteEmailExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new SiteEmailExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new SiteEmailExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.setting.siteemail.export_pdf', ['categories' => SiteEmail::all()]);
