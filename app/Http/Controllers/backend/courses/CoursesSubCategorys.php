...
use App\Exports\ExportData\CourseSubcategoryExport;
use ZipArchive;
...
class CoursesSubCategorys extends Controller
{
     /**
     * =============================================================
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = CourseSubCategory::where('status',1)->where('course_sub_category_name','LIKE',"%$search%")
            ->orWhere('course_sub_category_title','LIKE',"%$search%")->orWhere('course_sub_category_des','LIKE',"%$search%")->get();
        }else{
            $all = CourseSubCategory::where('status',1)->get();
        }
        return view('backend.courses.coursesubcategory.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = CourseSubCategory::get()->count();
        $latestPost = CourseSubCategory::latest()->first();
        $courseCategory = CourseCategory::get();
        return view('backend.courses.coursesubcategory.add',compact('totalpost','latestPost','courseCategory'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= CourseSubCategory::with(['metaData'=>function($query){
        ...
        return view('backend.courses.coursesubcategory.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        $totalpost  = CourseSubCategory::get()->count();
        $latestPost = CourseSubCategory::latest()->first();
        $data= CourseSubCategory::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        $courseCategory = CourseCategory::get();
        return view('backend.courses.coursesubcategory.edit',compact('totalpost','latestPost','data','courseCategory'));
    }
   ...
   /**
    ...
    public function insert(Request $request){
        /**--- validation code -- */
        $request->validate([
              'course_sub_category_name'  => ['required','unique:' . CourseSubCategory::class],
                'course_sub_category_title'=> 'required',
                'course_sub_category_desc'=> 'required',
            ],[
                'course_sub_category_name.required'=> 'Course Category Name is Required !',
                'course_sub_category_name.unique' => 'This Course Category Name already exists!',
                'course_sub_category_title.required'=> 'Course Category Title is Required !',
                'course_sub_category_desc.required'=> 'Course Category Description is Required !',
            ]
        );
        ...
        $categoryname = strtolower($request->course_sub_category_name) ;
        ...
        if(!empty($user_input_url)){
            ...
            $exists = CourseSubCategory::where('url', $url)->exists();
            if($exists){
        ...
        $insert = CourseSubCategory::create([
            'course_category_id'=>$request->course_category_id,
            'course_sub_category_name'=>$request->course_sub_category_name,
            'course_sub_category_title'=>$request->course_sub_category_title,
            'course_sub_category_des'=>$request->course_sub_category_desc,
            'slug'=>$slug,
            'url'=>$url,
            'creator_id' => $creator,
   ...
   /**
    ...
    public function update(Request $request){
           /**--- validation code -- */
           $request->validate([
            'course_sub_category_name'  => 'required',
              'course_sub_category_title'=> 'required',
              'course_sub_category_desc'=> 'required',
          ],[
              'course_sub_category_name.required'=> 'Course Sub Category Name is Required !',
              'course_sub_category_title.required'=> 'Course Sub Category Title is Required !',
              'course_sub_category_desc.required'=> 'Course Sub Category Description is Required !',
          ]
      );
    ...
    if(!empty($user_input_url)){
        $url = Str::slug($user_input_url); // Output: "my-new-category-name"
        $exists = CourseSubCategory::where('url', $url)->where('id', '!=', $id)->exists();  // nijer id bad diye check korte hobe 
        if($exists){
            return back()->withErrors(['custom_url' => 'This URL already exists!'])->withInput();
        }
    }else{
    ...
    //---------category update -------//
    $update = CourseSubCategory::where('id',$id)->where('slug',$slug)->update([
        'course_category_id'=>$request->course_category_id,
        'course_sub_category_name'=>$request->course_sub_category_name,
        'course_sub_category_title'=>$request->course_sub_category_title,
        'course_sub_category_des'=>$request->course_sub_category_desc,
   ...
   /**
    ...
    public function softdelete($id){
        $data= CourseSubCategory::where('id',$id)->first();
        $data->delete();
        // ----Delete Successfully ----//
        if($data){
            flash()->success('Information Update Successfuly');
    ...
    **/
    public function restore($id){
        $data = CourseSubCategory::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = CourseSubCategory::onlyTrashed()->where('id', $id)->first();
    ...
    **/
    public function public_status($id,$slug){
        $published = CourseSubCategory::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = CourseSubCategory::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = CourseSubCategory::onlyTrashed()->get();
        return view('backend.courses.coursesubcategory.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = CourseSubCategory::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    CourseSubCategory::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    CourseSubCategory::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = CourseSubCategory::get();
       // return view('backend.courses.coursesubcategory.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.courses.coursesubcategory.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = CourseSubCategory::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.courses.coursesubcategory.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.courses.coursesubcategory.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new CourseSubcategoryExport, 'Course-Category.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new CourseSubcategoryExport, 'Course-Category.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new CourseSubcategoryExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new CourseSubcategoryExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.courses.coursesubcategory.export_pdf', ['categories' => CourseSubCategory::all()]);
