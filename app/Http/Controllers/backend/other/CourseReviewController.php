...
use App\Exports\ExportData\CourseReviewExport;
...
class CourseReviewController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = Course_review::where('status',1)->where('rating','LIKE',"%$search%")
           ->get();
        }else{
            $all = Course_review::where('status',1)->get();
        }
        return view('backend.other.course_review.index',compact('all'));
    }
    ...
    **/
    public function add(){
        ...
        return view('backend.other.course_review.add',compact('totalpost','latestPost','allcourse'));
    }
    ...
    **/
    public function view($id,$slug){
        ...
        return view('backend.other.course_review.view',compact('data'));
    }
   ...
   /**
    ...
    public function softdelete($id){
        $data= Course_review::where('id',$id)->first();
        $data->delete();
        // ----Delete Successfully ----//
        if($data){
    ...
    **/
    public function restore($id){
        $data = Course_review::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = Course_review::onlyTrashed()->where('id', $id)->first();
        if($data) {
            $data->forceDelete();
    ...
    **/
    public function public_status($id,$slug){
        $published = Course_review::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = Course_review::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = Course_review::onlyTrashed()->get();
        return view('backend.other.course_review.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'active') {
            $categories = Course_review::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    Course_review::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    Course_review::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = Course_review::get();
       // return view('backend.other.course_review.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.other.course_review.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = Course_review::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.other.course_review.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.other.course_review.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new CourseReviewExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new CourseReviewExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new CourseReviewExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new CourseReviewExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.other.course_review.export_pdf', ['categories' => Course_review::all()]);
