...
use App\Exports\ExportData\FaqExport;
use ZipArchive;
...
class FaqController extends Controller
{
   /**
    ...
    public function index(Request $request){
        ...
        if($search !=""){
            $all = Faq::where('status',1)->where('question','LIKE',"%$search%")
           ->get();
        }else{
            $all = Faq::where('status',1)->get();
        }
        return view('backend.other.faq.index',compact('all'));
    }
    ...
    **/
    public function add(){
        $totalpost  = Faq::get()->count();
        $latestPost = Faq::latest()->first();
        return view('backend.other.faq.add',compact('totalpost','latestPost'));
    }
    ...
    **/
    public function view($id,$slug){
        $data= Faq::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.other.faq.view',compact('data'));
    }
    ...
    **/
    public function edit($id,$slug){
        ...
        $latestPost = Faq::latest()->first();
        $data= Faq::where('status',1)->where('id',$id)->where('slug',$slug)->firstOrFail();
        return view('backend.other.faq.edit',compact('totalpost','latestPost','data'));
    }
   ...
   /**
    ...
    //---------category update -------//
    $update = Faq::where('id',$id)->where('slug',$slug)->update([
        'question'=>$request->question,
        'answer'=>$request->answer,
        'editor_id' => $editor,
    ...
    **/
    public function public_status($id,$slug){
        $published = Faq::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = Faq::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = Faq::onlyTrashed()->get();
        return view('backend.other.faq.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'heard_delete') {
            $categories = Faq::onlyTrashed()->whereIn('id', $ids)->get();
        ...
        if ($action === 'active') {
            $categories = Faq::whereIn('id', $ids)->get();

            if($categories){
                foreach($categories as $data){

                    Faq::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    Faq::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
        $categories = Faq::get();
       // return view('backend.other.faq.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.other.faq.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
       ...
       // return view('backend.other.faq.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.other.faq.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new FaqExport, 'info.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new FaqExport, 'info.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export CSV file
        Excel::store(new FaqExport, 'info.csv', 'public');
        
        // Export XLSX file
        Excel::store(new FaqExport, 'info.xlsx', 'public');
        
        // Export PDF file
        $pdf = Pdf::loadView('backend.other.faq.export_pdf', ['categories' => Faq::all()]);
